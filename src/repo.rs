...
impl AppRepo for TiberiusRepo {
    ...
    async fn costs_daily(
        ...
        // 経費ﾏｽﾀ の主キーは 経費種別C + 経費C の複合だが、**引き当ては 経費C 単独で
        // 行う**。実機の 51 行を数えて 経費C が全行で一意と確認済み (01+0621=軽油費、
        // 02+0631=車検整備費 …) で、複合にすると逆に、明細側の 経費種別C が
        // マスタとずれている行で名前だけ空になる。経費種別ﾏｽﾀ は 経費種別C 単独キー。
        ...
        let query = format!(
            "SELECT TOP {} \
             ...
             ISNULL(t.[経費C], ''), \
             ISNULL((SELECT TOP 1 m.[経費N] FROM [経費ﾏｽﾀ] m WHERE m.[経費C] = t.[経費C]), ''), \
             ISNULL(t.[経費種別C], ''), \
