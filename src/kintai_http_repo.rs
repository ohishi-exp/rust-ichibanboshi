...
mod tests {
    ...
    /// **本番の実物で形を測る** (Refs #205 の 37)。オンプレ 23 桁 = 運行NO (22) +
    /// **対象CD** (1)、GCP 22 桁 = 運行NO そのもの。運行NO の中身は
    /// `YYMMDDHHMMSS` (12) + ゼロ埋めの車番 (10)。
    ///
    ...
    /// 畳むのが正しい (struct docs の実測)。数えているのは「キーだけでは対象CD
    /// 違いと別運行を区別できないので、0 でなければ中身を見よ」という合図。
    #[test]
