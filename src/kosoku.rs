...
/// 時刻 `at` が運行の中か。**後ろに最初に来る運行境界**で決める (Refs #182)。
///
/// 素直には「直前に見えた境界が `運行開始` なら中」だが、`time_card_dtako` の
/// `運行開始` は**当てにならない**:
///
/// - 運行が**月をまたいで続く**と当月データに頭が無い (実測 1041 / 2026-03: 2/28 に
///   始まった運行が 03-07 00:02 の `運行終了` まで続く)
/// - 頭の行が**丸ごと欠けている**日がある (実測 1248 / 2026-05-01〜02: `運行終了` が
///   2 つ続き、間の `運行開始` が無い)
///
/// どちらも「運行の外」に倒れると丸めが端点床から経過床へ入れ替わり、区分 1 つあたり
/// 1 分ずつ紙より小さくなる。**先に `運行終了` が控えているならそこは運行の中**という
/// 後ろ向きの判定なら、頭の有無に依存しない。後ろに境界が無ければ直前の境界で決める
/// (最後の `運行終了` から終業までの尻尾は運行の外)。
