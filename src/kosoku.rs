...
fn rest_spans(events: &[Event]) -> Vec<(NaiveDateTime, NaiveDateTime)> {
    let mut rests: Vec<(NaiveDateTime, NaiveDateTime)> = events
        ...
        .filter(|(s, e)| e > s)
        .collect();
    // **他の休息を丸ごと含む休息は捨てる** (Refs ohishi-exp/nuxt-dtako-admin#501)。
    // 休息の中でまた休息することはあり得ない — 中に別の休息があるなら、外側は
    // 運行を閉じ忘れた車輌の張り付きイベント。実測 (1731 藤田): 02-27 10:43 →
    // 03-23 09:40 の **24 日間の休息** (運行 260221 の閉じ忘れ) が正常な休息 15 本を
    // 包んでいて、[`split_long_shift`] がこれを「勤務の中の休息」として使い
    // 03-21 07:04→13:35 の勤務を丸ごと食っていた (紙 391 に対し行ごと消えた)。
    //
    // **長さでは切らない** — 連休の 100〜180 時間の休息は実データに普通にある
...
fn unpunched_ops_shift(
    ...
    // 終わりは**運行が窓の中で終わったかどうか**で決める (Refs #167)。かつて
    // `ALL_EVENTS_SQL` がイベント名を 4 種に絞っていた頃、「最後のイベントの終わり」
    // (latest) は経路で値が割れた (実測 1072 立山 03-25: 1 名指定 660 / 全乗務員 553)。
    // 絞りは撤回済みだが、運行の終わりは点イベントで判定するのが意味的にも正しい
    // (休憩の終わりで切ると帰庫前の移動が落ちる) のでこのまま。
...
/// 時刻 `at` が運行の中か。**前後どちらかが運行を示せば中** (Refs #182)。
///
/// `time_card_dtako` の運行境界は**片側が欠けることがある**ので、片方だけを見ると
/// 倒れる:
///
/// - 運行が**月をまたいで続く**と当月に頭が無い (実測 1041 / 2026-03)
/// - **頭の行が欠ける**日がある (実測 1248 / 2026-05-01〜02: `運行終了` が 2 つ続く)
/// - **尻尾の行が欠ける**日がある (実測 1541 / 2026-04-22〜29: `運行開始` が 2 つ続く)
///
/// 「運行の外」に倒れると丸めが端点床から経過床へ入れ替わり、区分 1 つあたり 1 分ずつ
/// 紙より小さくなる。**直前が `運行開始` か、次が `運行終了` なら中** — どちらか一方でも
/// 運行の存在を示せば十分で、両方が外を指す (直前が `運行終了` かつ次が `運行開始`) ときだけ
/// 運行と運行の間。境界が片側に無い場合は在る側だけで決まる。
fn in_run_at(events: &[Event], at: NaiveDateTime) -> bool {
    let is_bound = |e: &&Event| e.state == "運行開始" || e.state == "運行終了";
    let prev_is_start = events
        .iter()
        .filter(is_bound)
        .filter(|e| e.start <= at)
        .max_by_key(|e| e.start)
        .is_some_and(|e| e.state == "運行開始");
    let next_is_end = events
        .iter()
        .filter(is_bound)
        .filter(|e| e.start > at)
        .min_by_key(|e| e.start)
        .is_some_and(|e| e.state == "運行終了");
    prev_is_start || next_is_end
}
...
mod tests {
    ...
    #[test]
    fn a_missing_run_end_row_still_counts_as_inside_the_run() {
        // 実測 1541 吉田 / 2026-04-22〜29: `運行開始` が 2 つ続き、間の `運行終了` の行が
        // 無い。次の境界 (`運行開始`) だけで決めると「運行の外」に倒れて経過床になる。
        // 05:50:34 → 08:19:27 は端点床で 149、経過床では 148
        let rows = vec![
            dtako("2026-06-01 06:28:42", "運行開始"),
            ev("2026-06-01 07:00:00", "2026-06-02 05:50:34", "休息"),
            ev("2026-06-02 05:50:34", "2026-06-02 08:19:27", "運転"),
            ev("2026-06-02 08:19:27", "2026-06-02 12:00:00", "休息"),
            dtako("2026-06-03 07:22:54", "運行開始"),
        ];
        let d = daily_summary(&rows, "2026-06", &KosokuParams::default())
            .into_iter()
            .find(|d| d.date == "2026-06-02")
            .expect("06-02 の勤務");
        assert_eq!(d.restraint_minutes, 149);
    }

    #[test]
    fn a_missing_run_start_row_still_counts_as_inside_the_run() {
        // 実測 1248 松本 / 2026-05-01〜02: `time_card_dtako` に `運行終了` が 2 つ続き、
        // 間の `運行開始` の行が無い。直前の境界 (`運行終了`) で決めると「運行の外」に
        // 倒れて経過床になり、区分ごとに 1 分ずつ紙より小さくなっていた。
        // 06:02:41 → 09:06:23 は端点床で 184、経過床では 183
        let rows = vec![
            dtako("2026-06-01 08:44:54", "運行終了"),
            ev("2026-06-01 09:00:00", "2026-06-02 06:02:41", "休息"),
            ev("2026-06-02 06:02:41", "2026-06-02 09:06:23", "運転"),
            ev("2026-06-02 09:06:23", "2026-06-02 12:00:00", "休息"),
            dtako("2026-06-02 15:10:24", "運行終了"),
        ];
        let d = daily_summary(&rows, "2026-06", &KosokuParams::default())
            .into_iter()
            .find(|d| d.date == "2026-06-02")
            .expect("06-02 の勤務");
        assert_eq!(d.restraint_minutes, 184);
    }
    ...
    #[test]
    fn a_rest_containing_other_rests_is_dropped() {
        // 乗務員 1731 藤田 / 2026-03 の形 (Refs ohishi-exp/nuxt-dtako-admin#501)。
        // 運行を閉じ忘れた車輌が 24 日間の休息イベント (02-27→03-23) を残し、その中に
        // 正常な休息 15 本が入っていた。split_long_shift がこの張り付きを「勤務の中の
        // 休息」として使い、03-21 07:04→13:35 の勤務が丸ごと消えて 03-23 09:40 始まり
        // の残骸 (1703 分) ができていた。中に別の休息を含む休息は捨てる
        let rows = vec![
            // 張り付き (下の休息 2 本を丸ごと含む)
            ev("2026-06-01 10:43:00", "2026-06-23 09:40:00", "休息"),
            ev("2026-06-19 11:17:00", "2026-06-20 08:35:00", "休息"),
            ev("2026-06-20 10:07:00", "2026-06-21 07:04:00", "休息"),
            dtako("2026-06-21 13:35:00", "運行終了"),
            // 張り付きの元の運行がようやく閉じる点イベント (勤務の境界にしない)
            dtako("2026-06-23 09:40:00", "運行終了"),
            dtako("2026-06-24 08:32:00", "運行開始"),
            dtako("2026-06-24 14:03:00", "運行終了"),
            dtako("2026-06-25 08:07:00", "運行開始"),
            ev("2026-06-25 19:04:00", "2026-06-26 05:50:00", "休息"),
        ];
        let days = daily_summary(&rows, "2026-06", &KosokuParams::default());
        assert_eq!(
            days.iter()
                ...
                .collect::<Vec<_>>(),
            vec![
                ("2026-06-20 08:35:00", "2026-06-20 10:07:00"),
                ("2026-06-21 07:04:00", "2026-06-21 13:35:00"),
                ("2026-06-24 08:32:00", "2026-06-24 14:03:00"),
                ("2026-06-25 08:07:00", "2026-06-25 19:04:00"),
            ],
        );
        assert!(days.iter().all(|d| !d.over_24h));
    }
    ...
    #[test]
    fn identical_duplicate_rests_do_not_drop_each_other() {
        // 取り込み 2 重の完全同一行は `drop_duplicate_rows` の担当だが、素通り
        // してきても「含む」判定 (o != r) で互いを消さないこと
        let rows = vec![
            ev("2026-06-01 16:19:00", "2026-06-02 04:42:00", "休息"),
            ev("2026-06-01 16:19:00", "2026-06-02 04:42:00", "休息"),
            ev("2026-06-02 16:18:00", "2026-06-03 06:01:00", "休息"),
