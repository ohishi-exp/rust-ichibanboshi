...
pub async fn payroll(
    ...
    // 対象年度 DB の存在とアクセス可否を先に確認し、権限抜け (restore 由来) を
    // 500 でなく明示的なエラーで返す (#82 受け入れ条件)
    let databases = repo.list_kydata_databases().await.map_err(map_repo_err)?;
    match databases.iter().find(|(name, _)| name == &db) {
        None => {
            return Err(err(
                StatusCode::NOT_FOUND,
                format!("{db} がありません (この会社×年度の給与データは未作成です)"),
            ));
        }
        Some((_, has_access)) if *has_access != Some(1) => {
            return Err(err(
                StatusCode::SERVICE_UNAVAILABLE,
                format!(
                    "{db} にアクセスできません (データ復旧で作られた DB は権限の再付与が必要です)"
                ),
            ));
        }
        Some(_) => {}
