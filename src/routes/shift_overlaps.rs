...
//!
//! 最低賃金の検証タブの条件 3 の前の方式 (「乗務員 × 暦日の拘束の合計 > 1440」) は、
//! 8:00〜18:00 と 10:00〜20:00 のように **合計が 24h を超えないかぶり**を見逃す。
//! ここはその置き換えとして、`kintai.shifts` (`start_at` / `end_at`、
...
//! stale になる (`stale_months.rs` / `unko_gaps.rs` と同じ分類)。
//!
