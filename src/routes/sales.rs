...
pub fn split_and_sort_yoy(
    ...
    (
        pos.into_iter().take(limit).collect(),
        neg.into_iter().take(limit).collect(),
    )
}
...
pub async fn monthly(
    ...
    let (source_table, current, prev) = repo
        .monthly(
            &from_date,
            &to_date,
            &prev_from,
            &prev_to,
            params.exclude_dept.as_deref(),
            params.include_dept.as_deref(),
        )
        .await
        .map_err(map_repo_err)?;
    Ok(Json(ApiResponse {
        source_table,
        data: build_monthly_sales(&current, &prev),
    }))
}
...
pub async fn by_department(
    ...
    let from_date = format!(
        "{}-01",
        params.from.unwrap_or_else(|| "2025-04".to_string())
    );
    let to_date = format!("{}-01", params.to.unwrap_or_else(|| "2026-03".to_string()));
    let raw = repo
        .by_department(&from_date, &to_date)
        .await
        .map_err(map_repo_err)?;
    Ok(Json(ApiResponse {
        source_table: "部門別月計 + 部門ﾏｽﾀ".to_string(),
        data: build_department_sales(&raw),
    }))
}
...
pub async fn by_customer(
    ...
    let raw = repo
        .by_customer(&from_date, &to_date, limit)
        .await
        .map_err(map_repo_err)?;
    Ok(Json(ApiResponse {
        source_table: "得意先別月計 + 得意先ﾏｽﾀ".to_string(),
        data: build_customer_sales(&raw),
    }))
}
...
pub async fn customer_yoy(
    ...
    let to_date = format!("{}-01", to);
    let (prev_from, prev_to) = calc_prev_period(&from, &to);
    let (cur_map, prev_map) = repo
        .customer_yoy_data(&from_date, &to_date, &prev_from, &prev_to)
        .await
        .map_err(map_repo_err)?;
...
pub async fn yoy(
    ...
    Ok(Json(ApiResponse {
        ...
        data: build_yoy_comparison(&current, &prev),
    }))
}
...
pub async fn daily(
    ...
    let exclude_pattern = exclude_dept
        .as_ref()
        .map(|d| format!("%{}%", d))
        .unwrap_or_default();
    ...
    Ok(Json(ApiResponse {
        source_table: if dept_label.is_empty() {
            ...
            format!("運転日報明細 [{}, {}除く]", ml, dept_label)
        },
...
pub async fn customer_trend(
    ...
    let to_date = format!("{}-01", params.to.unwrap_or_else(|| "2026-03".to_string()));
    let limit = params.limit.unwrap_or(20);
    let (top, monthly_raw) = repo
        .customer_trend_data(&from_date, &to_date, limit)
        .await
        .map_err(map_repo_err)?;
    Ok(Json(ApiResponse {
        source_table: "得意先別月計 + 得意先ﾏｽﾀ".to_string(),
        data: build_customer_trend(&top, &monthly_raw),
    }))
}
...
pub async fn customer_detail(
    ...
    let (customer_name, raw) = repo
        .customer_detail_data(&params.code)
        .await
        .map_err(map_repo_err)?;
    Ok(Json(ApiResponse {
        source_table: "得意先別月計 + 得意先ﾏｽﾀ".to_string(),
