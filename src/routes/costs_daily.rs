...
//! 素直に足すと、その行が当たった 1 運行だけが赤くなる。按分するか外すかは消費側の
//! 判断なので、**判断できる材料として区分をそのまま返す** (`request_kind` と同じ方針)。
...
pub struct RawCostsDailyRow {
    ...
    /// `経費C` → `経費ﾏｽﾀ.経費N` (表示名)。マスタは同一 `経費C` の複数行があり得るので
