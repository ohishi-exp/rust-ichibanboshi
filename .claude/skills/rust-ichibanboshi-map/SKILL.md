...
- **運行の中かどうかは `in_run_at`** — **後ろに最初に来る境界**が `運行終了` なら中、という
  後ろ向きの判定。`time_card_dtako` の `運行開始` は当てにならないため: 運行が月をまたいで
  続くと当月に頭が無く (実測 1041 / 2026-03)、頭の行が丸ごと欠けた日もある (実測 1248 /
  2026-05-01〜02 は `運行終了` が 2 つ続き間の `運行開始` が無い)。どちらも「外」に倒れると
  丸めが端点床から経過床へ入れ替わり、**区分 1 つあたり 1 分ずつ紙より小さくなる**。後ろに
  境界が無ければ直前の境界で決める (最後の `運行終了` → 終業の尻尾は運行の外)
