...
- **運行の中かどうかは `in_run_at`** — **直前が `運行開始` か、次が `運行終了` なら中**。
  `time_card_dtako` の運行境界は**片側が欠ける**ので片方だけ見ると倒れる: 月跨ぎで頭が無い
  (1041 / 2026-03)、頭の行が欠ける (1248 / 2026-05-01〜02 は `運行終了` が 2 つ続く)、
  尻尾の行が欠ける (1541 / 2026-04-22〜29 は `運行開始` が 2 つ続く)。「外」に倒れると丸めが
  端点床から経過床へ入れ替わり **区分 1 つあたり 1 分ずつ紙より小さくなる**。両方が外を指す
  (直前が `運行終了` かつ次が `運行開始`) ときだけ運行と運行の間
- **紙が引く `運行開始 → 始業` は `minus_unko_by_date`** (view=compare、Refs #546)。紙は打刻より
  先に運行を始めた日、その頭を日計から**引く** (`_make_minus_unko_day`) のでこちらより小さくなる。
  紙の再現値には元から効いていたが、突合の cause 候補には**実額**が要る — 無いと他の実額と
  組み合わせられず、複合の日が drift (cause `rounding`) の受け皿へ落ちる (実測 1729 / 2026-01-09
  の −15 = ours-outside 6 + minus_unko 9)。紙と同じく **TC_DC が値を持つ日だけ**返す
- **深夜を跨ぐ継ぎ目の暦日配分の差は `gap_midnight_by_date`** (view=compare、Refs #546)。紙は
  `運行終了 → 運行開始` の対を**丸ごと運行開始の日**へ載せる (`tc_dc_daily` の「同日の縛りが
  無い」) が、こちらは暦日で割る — 0 時より前の分だけ前日へ回る。**月合計は変わらず置き場所が
  違うだけ**なので符号は前日が負・運行開始日が正。実測 1536 / 2026-06-10 23:51:55 → 06-11
  07:03:07 (車両の乗り換え): 紙は 431 分を全部 06-11、こちらは 06-10 に 8 + 06-11 に 423。
  `paper_outside_by_date` の深夜跨ぎ処理 (#190) は**勤務の外**の継ぎ目だけが対象なので、
  継ぎ目が拘束の**中**にあるこの形は拾えない
- **休息の中で運行が始まったら休息はそこで閉じる** (`close_rest_at_run_start`、Refs #182)。
  車両を乗り換えると前の車両の記録に休息が開いたまま次の運行が始まる — 実測 1029 /
  2026-05-13 は車両 1867 の休息 813 分の中で 03:32 から車両 2409 を走っており、丸ごと引くと
  拘束 118 分 (紙は 745)。**入れ子の休憩とは区別できる** — 同じ運行の休憩には `運行開始` が
  挟まらない
...
- **打刻の受け口のテナントは `X-Tenant-ID`**、`[kintai_push] tenant_id` ではない。
  relay が KV (`dtako-relay-config` の `dtako_accounts`、`comp_id → tenant_id`) から
  引いた値が正で、alc へも同じヘッダで渡している。auth-worker が JWT から注入するのは
  **ブラウザ経路だけ** — relay は browser JWT を持たず、管理者の tenant と `comp_id` の
  tenant が一致する保証も無い (`alc-internal-upload.ts` が不採用の理由を記録)。
  設定の `tenant_id` は**任意で、書けば pin** (ヘッダと食い違えば 403)。ヘッダも pin も
  無ければ 400 — 既定のテナントへ落とすと別テナントに静かに積まれる
- **CLI (`push`/`recalc`/`sync`) にはヘッダが無いので `tenant_id` は必須。** `main.rs` が
  起動前に弾く。値は `rust-alc-api` の `alc_api.tenants.id` で、書いた場合は
  `[kintai_events] tenant_id` と一致必須 (起動時に検査)
...
`tests/kintai_{push,fold}_pg_test.rs` が**実 Postgres** に対して 29 件。`ci.yml` の
