...
- **秒は「経過時間を切り捨て」で落とす** (既定 `truncate_elapsed`、`RestraintRounding`)。
  紙のタイムカード表 (社内 CakePHP) は `date_diff()->h*60 + ->i` で**差の秒を捨てる**ので、
  始業 09:00:30 / 終業 17:00:20 は 479 分。従来の「両端をそれぞれ切り捨てる」
  (`floor_endpoints`、`kosoku.restraint_rounding` で戻せる) だと 480 分で **1 分大きく**なり、
  これが突合の 1 分差の正体だった (実測 2026-03: 両方に居る乗務員の 467 日中 204 日が 1〜2 分差。
  Refs ohishi-exp/nuxt-dtako-admin#501)。**削るのは終業側**なので `拘束 = 実働 + 休憩` は保たれ、
  削った 1 分に乗る終業打刻も `punch_window_end` で拾う
