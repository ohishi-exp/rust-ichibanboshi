...
### Phase 2 — ブラウザ側のウィンドウ合成 (**本当の空白地帯。ここが本体**) ✅ 完了 (2026-08-17)

**実機 (ohishi-srv) で受け入れ条件 2・3・4 をすべて確認済み。**
IronRDP fork の `claude/mystifying-faraday-7874bc` に 4 コミット。
成果と踏んだ罠は末尾の「Phase 2 の結果」を参照。
...
### Phase 4 — 周辺チャネル (実装ではなく統合) 🔧 印刷は到達済み

印刷は Phase 2 の実機検証中に前倒しで通した (PDF がブラウザに戻る)。
クリップボードは未検証。
...
## Phase 2 の結果 (2026-08-17)

### できたこと

一番星 運送業システムが**ブラウザ内に描画され、操作でき、印刷が手元に戻る**ところまで到達。
実機 (ohishi-srv / Windows Server 2016 RDS) で確認した。

| 受け入れ条件 | 結果 |
|---|---|
| 2. 子画面を開閉してもセッションが操作不能にならない | ✅ |
| 3. 3 枚目のウィンドウ (印刷ダイアログ) も出る | ✅ |
| 4. 入力が通る | ✅ |
| (追加) 印刷がブラウザに戻る | ✅ PDF |

### 上流の地形が変わっていた

Phase 1 の直前に RemoteApp 関連が 4 本マージ済みだった (#1631 / #1636 / #1637 / #1641)。
とくに **#1641 が ActiveX 面で同じことをやっていて、テスト付きのフィールドパーサがある**。
同一 repo・同一ライセンスなので参照でき、Phase 2 の見積もりは下がった。

ただし **ironrdp-web だけは置き去り**で、空白地帯は本物だった。

**上流のパーサにバグを見つけた**: `parse_projected_rail_window_order` の
`bool::then(...)?` パターンは、フィールドが**無い**とき関数ごと早期 return する。
OWNER を持たない増分オーダー (窓の移動・リサイズ) が丸ごと捨てられる。移植版では直した。

### 実機でしか出なかった罠 (どれも設計段階では見えなかった)

1. **RAIL キューが解放されずアプリが起動しない** — `RailClient` は Execute を
   ハンドシェイクとデスクトップ同期の両方が済むまで溜めるが、サーバーは同期を通知しない
   ことがある。`ironrdp-client` と同じく 250ms のタイマーで
   `release_queued_after_handshake()` を叩く必要がある
2. **デスクトップ再同期で画面が全消しになる** — Desktop Information Order は
   **ウィンドウ一覧**を送り直す通知で、**ピクセルは送り直されない**。一覧が空になった
   瞬間を「誰も覆っていない」と解釈すると、まだ画面にある窓ごと黒く塗り潰す
3. **矩形の指数爆発** — カバー 1 枚ごとに矩形を 4 分割して畳み込むと窓 N 枚で 4^N。
   16 枚でタブが OOM。水平バンドごとに X 区間の差集合を取る方式に置き換えた
4. **ログ量でタブが OOM** — ウィンドウオーダーは毎秒何百回も届く。ホットパスのログは
   `trace!` に落とし、文字列生成自体もレベル判定でガードする
5. **Refresh Rect PDU は使えない** — 消しすぎを自己修復させる案は、Windows Server 2016 が
   `An invalid Refresh Rect PDU has been received` で**即座に切断**してくる。
   上流が `request_full_redraw` で Suppress Output を優先しているのと同じ理由

### 未解決

**PDF タイトルの文字化け** — サーバー側の Microsoft Print to PDF が `/Title` に生の
UTF-8 を書いている (仕様は PDFDocEncoding か UTF-16BE + BOM)。同じ長さのまま書き換える
