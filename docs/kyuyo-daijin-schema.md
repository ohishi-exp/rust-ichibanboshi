...
- 項目番号 001〜017 (17項目) = 勤怠系 (出勤日数・公休日数・有休日数・欠勤日数・残業時間・
  遅刻回数 等) → `KYUYO.KINDATA0000, KINDATA0100, ..., KINDATA1600` (17列、100刻み、当月分) と
  `KINDATA0001, ..., KINDATA1601` (17列、別集計。累計等と推測、未確定)
  - **`KINDATA*` の値は全社・全項目で ×100 の固定小数** (`raw / 100` = 実数、#103 で 4 社の
    実データにより確定)。**「回数」系も例外ではない** — 遅刻回数の生値 `800` は 8 回で、
    割らずに使うと 800 回になる。実測: 残業時間 `5900`=59.00h / 有休日数 `50`,`150`,`250`
    = 0.5,1.5,2.5 日 (半休も整合) / 遅刻回数 `800`=8 回 / 残業時間 `13750`=137.50h
  - **`KOUMOKU.NUMMODE` (1〜4) はスケール区分ではない。** 「表示モード」という名前で
    スケールの区分に見えるうえ、同じ項目 (残業時間) でも会社によって 2 だったり 4 だったり
    するため紛らわしいが、これは**給与大臣 UI 側の表示書式** (会社ごとに個別設定される属性) で、
    格納スケールとは無関係。値によらず一律 `/100` でよい
...
### 項目ごとの支給/控除の判定 → `KAZEI` (2026-07-25 実証、Refs #93)

`KOUMOKU` には `KUBUN`(1〜5)・`KAZEI`(課税区分)・`GENGAKU`・`MEISAI` 等のフラグ列がある。
当初「これらから機械的に判定するのは信頼できない」と書いたが、**`KAZEI` だけは使える**
ことを実データで確認した (0100社 MONTH=5)。

| フラグ | 支給/控除の判定に使えるか |
|---|---|
| `KUBUN` (1〜5) | **使えない**。支給 (基本給=3, 役職手当=1, けん引手当=2) と控除 (立替金=2, その他控除=1, 被服代=2) が同じ値に混在する |
| `GENGAKU` | **バケツ分けには使えないが、符号には使う**。`=1` は「時間修正控除」「遅早控除」「その他減額」等ごく一部。既に `SOSHIKYU` から差し引かれているので、**控除側へ倒すのは誤り** (`deduction_total` は `HOKEN+ZEI+SHOKOUJO` が正で減額項目を含まない) / **支給側で符号を反転する**のが正しい (下記) |
| **`KAZEI`** | **使える**。`1`/`2` = 支給 (`2` は通勤手当の非課税枠)、`0` = 控除 |
| **`MEISAI`** | `=1` は**単価項目**。支給でも控除でもないので両方から除外する |

**検証**: `KAZEI` ベースで分類した支給項目の合計を `SHUKEI1.SOSHIKYU{NN}` と突合 →
**0100社 MONTH=5 の全52名で完全一致 (差異0件)**。

合計値だけが必要なら次項の `SHUKEI1` を使えばよいが、**項目ごとの区分**が要る場合
(給与比較が項目を割増基礎/最低賃金の 5 区分に割り当てる、Refs nuxt-dtako-admin#369) は
`KAZEI` で分ける。

### 減額項目の符号 → `GENGAKU=1` は支給側でマイナス (2026-07-25 実証、Refs #87)

`MONEY` には**絶対値が正**で入るが `SHUKEI1.SOSHIKYU` は既に差し引き済み。よって
`KAZEI` で支給に振ったあと `GENGAKU=1` の項目だけ符号を反転して合計する。

**実測**: 0200社 2026-06 で「時間修正控除」を持つ 8 名がちょうど
`支給合計 − SOSHIKYU = 時間修正控除 × 2` だけ過大になっていた (会社全体でこの項目を
持つのはその 8 行のみで、8 行とも不一致)。0100/0300/0400 には当月該当項目が無く差異 0 件。
...
`try_get::<i32>` は型厳格なので、`CAST(... AS int)` を付けずに読むと `Err` →
既定値 0 に化け、**全項目が控除に落ちて `payments` が空になる**。`KCODE` (#86) と
同じ罠で、本番で実際に起きた (#95)。`KYDATA*` から整数を読むクエリには必ず `CAST` を付ける。

### 単価 (基本単価・残業単価) → `MEISAI=1` の `KOUMOKU` 項目 (Refs #93)

単価は別テーブルではなく **`MONEY00..79` の枠内に項目として入っている**:

| 項目番号 | TAIKEIKOUNO (体系1) | KOUMOKU.NAME | MONEY列 |
|---|---|---|---|
| 078 | `01078` | 残業単価 | `MONEY60` |
| 079 | `01079` | 通勤単価 | `MONEY61` |
| 080 | `01080` | 基本単価 | `MONEY62` |
| 081 | `01081` | 休日単価:1日 | `MONEY63` |

- 他の支給/控除項目とは `MEISAI=1` フラグで区別される
- **値は ×100 の固定小数** (`131800` = 1,318.00 円)
- 検証: `残業時間 (KINDATA0500=59.00h) × 残業単価 (MONEY60=1318.00円) = 77,762円` が
  `残業手当 (MONEY24)` と完全一致

→ `KINDATA0001`〜`1601` や `SHAIN2`〜`8` の新規調査は不要だった。
...
- [ ] `KINDATA0001`〜`KINDATA1601` (2つ目の勤怠系17列) の用途 (当月/累計の区別など)
      — なお単価はここではなく `MEISAI=1` の `MONEY` 項目だった (#93 で解決済み)
